...
<template>
  <div class="text-gray-800 antialiased flex justify-center py-6 flex-col sm:py-12">
    <div class="relative py-3 sm:w-96 sm:max-auto mx-auto">
      <span class="text-2xl font-light flex justify-center">Login to your account</span>
      <div class="mt-4 bg-white shadow-md rounded-lg">
        ...
        <div class="px-8 py-6">
          ...
          <p v-if="passwordIncorrect" class="text-red-400 font-semibold">Username or password is incorrect</p>
          ...
          <div class="flex justify-between items-baseline">
            <button @click="login()" type="submit" :disabled="loading" :class="{ 'opacity-50': loading }" class="mt-4 bg-indigo-500 text-white py-2 px-6 rounded-md hover:bg-indigo-600 active:bg-indigo-700">Login</button>
            <a href="#" class="text-sm hover:underline text-gray-400">Forgot Password</a>
          </div>
        </div>
      </div>
      ...
      <div class="text-gray-800 antialiased mt-4 bg-white shadow-md rounded flex justify-between items-baseline">
        <div class="px-8 py-6 flex w-full">
          <span class="text-lg mt-auto mb-auto">New to Todo?</span>
          <button @click="goToRegisterPage()" type="submit" class="ml-auto bg-green-500 text-white mt-1 mb-1 py-2 px-6 rounded-md hover:bg-green-600 active:bg-green-700">Register</button>
        </div>
      </div>
    </div>
  </div>
